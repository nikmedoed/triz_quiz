"""Static text messages and default titles."""

# Bot registration and naming
CURRENT_NAME = (
    "Текущее имя: {name}\n"
    "Отправьте новое имя одним сообщением.\n"
    "Или /cancel — оставить как есть."
)
ENTER_NAME = (
    "Введите имя для участия (одним сообщением).\n\n"
    "Или можно не вводить и использовать из профиля:"
)
PROFILE_NAME_BUTTON = "Взять из профиля"
PROFILE_NAME_NOT_AVAILABLE = "Не нашли имени в профиле. Напишите вручную."
PROFILE_NAME_NOT_WAITING = "Имя уже задано. Используйте /start, чтобы изменить."
NAME_EMPTY = "Имя пустое. Введите заново или /cancel."
NAME_SAVED = "Имя сохранено. Готово к участию."
NAME_UNCHANGED = "Ок, имя не меняем."
ASK_AVATAR = (
    "Выберите аватарку.\n\n"
    "- Взять из профиля, если доступно,\n"
    "- Отправьте стикер или эмодзи.\n\n"
    "Ещё быстрее: выбрать одну из случайных ниже."
)
AVATAR_NOT_WAITING = "Аватар уже установлен. Используйте /start, чтобы изменить."
AVATAR_INVALID = "Не получилось использовать этот вариант. Попробуйте другой."
AVATAR_SELECTED = "Вы выбрали {emoji}. Создаём аватар…"
AVATAR_PROFILE_BUTTON = "Взять фото профиля"
AVATAR_PROFILE_SELECTED = "Берём фото профиля. Создаём аватар…"
AVATAR_PROFILE_NOT_AVAILABLE = "Не нашли фото профиля. Пришлите эмодзи или стикер."

# Generic notifications
RESPONSES_CLOSED = "Ответы более не принимаются, вернитесь в общий зал."
IDEA_ACCEPTED = (
    "Идея принята!\n\n"
    "<i>Вы можете изменить ответ, отправив новое сообщение.\n"
    "Редактирование сообщений не поддерживается, скопируйте, измените и пришлите новое.</i>"
)
TEXT_NOT_ACCEPTED = (
    "Сейчас текстовые ответы не принимаются. Дождитесь команды модератора.\n"
    "Чтобы изменить имя: /start"
)
NOT_ANSWER_PHASE = "Сейчас не этап ответов."
ANSWER_UNCHANGED = "Ответ не изменён."
ANSWER_SAVED = "Ответ сохранён."
NOT_VOTE_PHASE = "Сейчас не этап голосования."
VOTE_REMOVED = "Голос снят."
VOTE_COUNTED = "Голос засчитан."
REGISTRATION_WAIT = "Ждём начала. Вы на экране регистрации."

# Open block prompts
OPEN_HEADER = "Предложите идею решения проблемной ситуации (открытый ответ)"
OPEN_INSTR = (
    "Пришлите ваш ответ в этот чат.\n"
    "- Учитывается один последний ответ\n"
    "- В свободной форме\n"
    "- Лаконично, важна скорость\n"
    "- Укажите логику решения, использванные приёмы, методы, обоснуйте"
)
VOTE_START = (
    "Ответы более не принимаются.\n\n"
    "<b>Начато голосование за идеи.</b>\n\n"
    "Выберите номера, которые считаете достойным и методологически обоснованным решением описанной проблемы.\n\n"
    "<i>Можно выбрать несколько, или ничего не выбирать.</i>"
)
VOTE_NO_OPTIONS = "Начато голосование за идеи. Для вас нет вариантов, ожидайте."
VOTE_FINISHED = (
    "Голосование завершено.\n"
    "Результаты на экране.\n"
    "Вы набрали {points} балл(ов)."
)

# Quiz prompts
QUIZ_HEADER = "Выберите один вариант ответа"
QUIZ_INSTR = (
    "Выберите\n"
    "- Наиболее подходящий вариант\n"
    "- Быстро, пока есть время\n"
    "- Можно изменить выбор"
)
NO_ANSWER = "Вы не ответили.\n\n"
CORRECT_PREFIX = "Верно! Вы получили {points} балл(ов).\n\n"
WRONG_ANSWER = "Вы ответили неверно.\n\n"

MCQ_RESULTS_INSTRUCTION = "Правильно проголосовали {correct} / {total}"

MULTI_RESULTS_INSTRUCTION = (
    "Ответили {total}, верно {full}, частично верно {partial}"
)

# Sequence prompts
SEQUENCE_HEADER = "Выберите варианты в правильном порядке"
SEQUENCE_INSTR = (
    "Нажимайте варианты в нужной последовательности.\n"
    "- Нажимайте последовательно для нумерации\n"
    "- Повторное нажатие снимает удаляет из выбранных\n"
    "- Кнопка \"Сброс\" очищает выбор\n"
    "- Если кнопка подвисла, подождите или нажмите ещё раз"
)
WRONG_SEQUENCE = "Порядок неверный.\n\n"
SEQUENCE_RESET = "Сброс"
SEQUENCE_PUBLIC_INSTR = "Участники выбирают порядок в боте"
SEQUENCE_STATS_CORRECT = "Верно"
SEQUENCE_STATS_WRONG = "Неверно"

# Leaderboard
LEADERBOARD = (
    "Викторина завершена.\n\n"
    "Набрано баллов: {score}.\n"
    "Ваше место: {place}.\n\n"
    "Среднее время ответа:\n"
    "{open_avg:.1f} c - открытый вопрос\n"
    "{quiz_avg:.1f} c - выбор варианта"
)

# Default titles for scenario steps
TITLE_REGISTRATION = "Регистрация"
TITLE_OPEN = "Гипотезы решения"
TITLE_QUIZ = "Квиз"
TITLE_SEQUENCE = "Последовательность"
MULTI_HEADER = "Выберите все верные варианты"
MULTI_INSTR = (
    "Выберите\n"
    "- Все подходящие варианты\n"
    "- Выбор можно отменить\n"
    "- Важно выбрать только верные"
)
TITLE_MULTI = "Множественный выбор"
TITLE_LEADERBOARD = "Финальная таблица"
PREVIEW_TITLE = "Предпросмотр сценария"
PREVIEW_INSTRUCTION = "Режим просмотра: ответы и результаты не сохраняются."
PREVIEW_EMPTY = "В файле сценария нет шагов для предпросмотра."
PREVIEW_BUTTON = "Посмотреть сценарий"
PREVIEW_HOME_BUTTON = "Викторина"
PREVIEW_RESET_BUTTON = "Сброс"
OPEN_PUBLIC_INSTR = "Отправляйте идеи боту. Здесь они пока не видны."
QUIZ_PUBLIC_INSTR = "Выберите один верный в боте"
MULTI_PUBLIC_INSTR = "Выберите все верные варианты в боте"

STAGE_VOTING_SUFFIX = "Голосование за идеи"
STAGE_RESULTS_SUFFIX = "Результаты голосования"
ANSWER_TOOK = "Ответ занял {time}"
NO_IDEAS = "Ответов нет"
SHOW_MORE = "Подробнее"
SHOW_LESS = "Короче"
